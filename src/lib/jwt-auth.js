 ...
 * Pure JavaScript ES Module compatible with Node.js, Cloudflare Workers & Browsers
 */
...
export function extractJwtFromRequest(request) {
...
export function isCourseAuthorized(payload, subjectCode) {
  if (!payload) return false;
  if (payload.role === 'admin' || payload.role === 'teacher') return true;
  if (!payload.allowedCourses || payload.allowedCourses.length === 0) return true;
